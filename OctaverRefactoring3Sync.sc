...
SynthDef(\joinOutput, { arg inBus, outBus = 0;
	var source;

	//Synthdef used to synchronize the synthetized signals
	//(which have more latency) with the original dry signal
...
var joinBus = Bus.audio(s, 1);

//Synth def variables

var octUp1MonoSD, octUp1PolySD, octDown1MonoSD, octDown1PolySD, chorusSD,lowPass;
var joinSD;
...
z = Synth(\octaverMain, [\inBus, inputBus, \octaveUpBus, octaveUp1BusMonophonic,
	\octaveDownBus, octaveDown1BusMonophonic, \outBus, joinBus],g
);

h = Synth(\readInputSignal, [\outBus, inputBus], g);


chorusSD = Synth.after(g, \chorus, [\inBus, chorusBus, \outBus, joinBus]);

joinSD = Synth.after(chorusSD, \joinOutput, [\inBus, joinBus]);
